...
std::optional<domain::CalendarMessageReference> SQLiteScheduleRepository::find_calendar_message(domain::CalendarId cid){std::scoped_lock lock(mutex_);auto s=prepare(db_,"SELECT guild_id,channel_id,message_id,displayed_week FROM calendar_messages WHERE calendar_id=?");sqlite3_bind_int64(s.get(),1,cid);if(sqlite3_step(s.get())!=SQLITE_ROW)return std::nullopt;return domain::CalendarMessageReference{cid,static_cast<domain::Snowflake>(sqlite3_column_int64(s.get(),0)),static_cast<domain::Snowflake>(sqlite3_column_int64(s.get(),1)),static_cast<domain::Snowflake>(sqlite3_column_int64(s.get(),2)),static_cast<std::size_t>(sqlite3_column_int(s.get(),3))};}
